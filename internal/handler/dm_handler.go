...
func (h *DMHandler) StartConversation(c *gin.Context) {
	...
	if err != nil { response.BadRequest(c, "Invalid target id"); return }
...
func (h *DMHandler) ListMessages(c *gin.Context) {
	userID, ok := authUser(c); if !ok { return }
	conversationID, ok := parseConversationID(c); if !ok { return }
	pg := queryIntDM(c, "page", 1)
	lm := queryIntDM(c, "limit", 20)
...
func handleDMError(c *gin.Context, err error) bool {
	...
	case err == service.ErrConversationNotFound:
...
func parseConversationID(c *gin.Context) (uuid.UUID, bool) { id, err := uuid.Parse(c.Param("id")); if err != nil { response.BadRequest(c, "Invalid conversation id"); return uuid.Nil, false }; return id, true }
func queryIntDM(c *gin.Context, key string, fallback int) int { v, err := strconv.Atoi(c.Query(key)); if err != nil || v < 1 { return fallback }; return v }
func totalPages(total int64, limit int) int { if limit <= 0 { return 0 }; return int((total + int64(limit) - 1) / int64(limit)) }
